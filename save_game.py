...
def save_game(game):
    ...
    payload = {
        ...
        "last_aggressor": {
            _enum_name(k): _enum_name(v) for k, v in game.last_aggressor.items()
        },
...
def load_game(game):
    ...
    game.defeated_countries = {
        _enum_from_name(Country, n) for n in payload.get("defeated_countries", [])
    }
    game.last_aggressor = {
    ...
    game.grid = [
        [Cell(x, y) for y in range(GRID_ROWS)] for x in range(GRID_COLS)
    ]
    for x in range(GRID_COLS):
        for y in range(GRID_ROWS):
            ...
            cell.discovered_by = {
                _enum_from_name(Country, n) for n in data.get("discovered_by", [])
            }
    ...
    for name, pdata in payload["players"].items():
        ...
        player.city_income_bonus = pdata.get("city_income_bonus", 0)
        player.bridge_hp_bonus = pdata.get("bridge_hp_bonus", 0)
        player.bridge_cost_reduction = pdata.get("bridge_cost_reduction", 0)
